...
先后依次是`euler，euler a，smea，smea beta，smea dyn beta`，注意`smea，smea beta，smea dyn beta`这三个我自己瞎jb写的，不代表novel ai官方逻辑，用的模型是noob eps 1.1，这里的测试样例是手动加代码而不是插件的，如果你想要复现需要手动加代码(见original.py)，调度器如果想要轮廓分明推荐simple，如果想要羽化边缘的感觉用karras
